...
app.post('/doSearch',async (req,res)=>{
    ...
    let results = await dbo.collection("Student").find({name: new RegExp(inputName,'i')}).toArray();
    res.render('allStudent',{model:results});
...
app.post('/doInsert',async (req,res)=>{
    ...
    let inputP1 = req.body.txtPhone1;
    let inputP2 = req.body.txtPhone2;
    let newStudent = {name : inputName, weigh:inputWeight, phone:{phone1:inputP1, Phone2:inputP2}};
    if(inputName.trim().length == 0){
        let modelError = {nameError:"ten khong de trang!", emailError:"Email khong phu hop"};
